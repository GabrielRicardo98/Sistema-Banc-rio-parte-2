...
def sacar():
    """Realiza a operação de saque, verificando o saldo com validação."""
    global saldo
    while True:
        try:
...
def depositar():
    """Realiza a operação de depósito com validação."""
    global saldo
    while True:
        try:
...
# Loop principal com menu e validação de entrada
while True:
    ...
    # Executar ação com base na opção
    if opcao == 1:
        consultar_saldo()
    elif opcao == 2:
        depositar()
